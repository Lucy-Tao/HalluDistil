...
#
# distill_and_eval_v3.sh <prompt_style> [run_tag] [epochs]
#
# distill -> generate -> judge in one job. Full distillation only.
# Target = teacher low_temp_response (T=0.1), read from the seed44
# judged file. NOT_ATTEMPTED targets are skipped inside distill.py.
#
# Usage:
#   sbatch distill_and_eval_v3.sh strict
#   sbatch distill_and_eval_v3.sh fewshot myrun 5

set -e
set -x

PROMPT_STYLE="${1:?Usage: sbatch distill_and_eval_v3.sh <strict|fewshot> [run_tag] [epochs]}"
RUN_TAG="${2:-}"
EPOCHS="${3:-3}"
...
JUDGED_OUTPUT_DIR="${PROJECT_DIR}/judged_data_distilled_seed44"
