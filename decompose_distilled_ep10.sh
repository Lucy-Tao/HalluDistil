#!/bin/bash
#SBATCH --job-name=decompose_distilled_ep10
...
#
# decompose_distilled_ep10.sh -- decompose + verify the ep10 distilled
...
RAW_INPUT="${GEN_DATA_DIR}/gen_factscore_bio_factscore_bio_distilled_student_ep10_distilled_ep10.jsonl"
ADAPTED_INPUT="${GEN_DATA_DIR}/answered_distilled_ep10.jsonl"
OUTPUT="${GEN_DATA_DIR}/claims_distilled_ep10.jsonl"
MODEL="gpt-5-mini"
...
echo "$(date -Iseconds) | job=${SLURM_JOB_ID:-none} | stage=decompose_distilled_ep10 | node=$(hostname)" >> "${MANIFEST}"
...
for attempt in $(seq 1 "${MAX_RETRIES}"); do
    ...
    echo "$(date -Iseconds) | job=${SLURM_JOB_ID:-none} | stage=decompose_distilled_ep10_retry | attempt=${attempt} | exit_code=${exit_code} | node=$(hostname)" >> "${MANIFEST}"
...
echo "===== [$(date)] Done ====="
echo "$(date -Iseconds) | job=${SLURM_JOB_ID:-none} | stage=decompose_distilled_ep10_done | node=$(hostname)" >> "${MANIFEST}"